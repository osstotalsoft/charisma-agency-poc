...
export async function loadMcpTools(
  ...
  try {
    ...
    if (missing.length > 0) {
      console.warn(
        `bridge-planner-agent: missing expected MCP tools: ${missing.join(", ")}`
      );
    }

    console.info(
      `bridge-planner-agent MCP tools loaded: ${Object.keys(toolMap).join(", ")}`
    );
    ...
    console.warn(
      "bridge-planner-agent: failed to load MCP tools — agent will run without them:",
      err
    );
