...
// Loads hcm-holiday-request and hcm-employee-profile MCP servers.
export async function loadMcpTools(
  ...
  if (Object.keys(serverConfigs).length === 0) {
    console.warn(
      "No MCP server URLs configured for holiday-bridge-agent — skipping MCP tool loading"
    );
    return {
  ...
  try {
    ...
    if (missing.length > 0) {
      console.warn(
        `holiday-bridge-agent: missing expected MCP tools: ${missing.join(", ")}`
      );
    }

    console.info(
      `holiday-bridge-agent MCP tools loaded: ${Object.keys(toolMap).join(", ")}`
    );
    ...
    console.warn(
      "holiday-bridge-agent: failed to load MCP tools — agent will run without them:",
      err
    );
