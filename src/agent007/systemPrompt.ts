...
Call internet_search to find Romanian public holidays for ${currentYear()}. Parse results into a HOLIDAY_SET: list of { date, name, dayOfWeek }. Filter out any holidays that have already passed (before ${today()}).
...
Call get_my_holiday_requests for year ${currentYear()}. Extract ALL booked date ranges (dateFrom → dateTo for every request) into a BLOCKED_SET.
...
- "December 15" with no year: use ${currentYear()} if that date is still in the future, otherwise ${currentYear() + 1}
- All dates must be FUTURE (on or after today)
- End date must be on or after start date

Calendar validation — only produce dates that actually exist:
- ${currentYear()} is ${currentYear() % 4 === 0 && (currentYear() % 100 !== 0 || currentYear() % 400 === 0) ? "" : "NOT "}a leap year (February has ${currentYear() % 4 === 0 && (currentYear() % 100 !== 0 || currentYear() % 400 === 0) ? 29 : 28} days)
